...
class Tensor:
    ...
    @classmethod
    def eye(cls, N, M=None, k=0, requires_grad=False, dtype=np.float32):
        data = init.eye(N, M, k)
        return cls(data, requires_grad=requires_grad, dtype=dtype)
    ...
    def normal(cls, shape, mean=0.0, std=1.0, requires_grad=False, dtype=np.float32):
        data = init.normal(mean, std, shape)
        return cls(data, requires_grad=requires_grad, dtype=dtype)
    ...
    def linear(self, weight, bias):
        ...
        """
        return F.Linear.apply(self, weight, bias)
    ...
    def __eq__(self, other):
        other = self._ensure_tensor(other)
        return self._create_tensor(self.data == other.data)
