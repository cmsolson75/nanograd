...
# Import your Tensor class and initializers here
# from your_module import Tensor

# Increasing sample size to reduce variability
SAMPLE_SIZE = 100000

@pytest.mark.parametrize("shape, mean, std", [
    ((SAMPLE_SIZE,), 0.0, 1.0),
    ((SAMPLE_SIZE,), 1.0, 2.0),
    ((SAMPLE_SIZE,), -1.0, 0.5)
])
def test_normal_initializer(shape, mean, std):
    my_tensor = Tensor.normal(shape, mean=mean, std=std).data
    torch_tensor = torch.empty(shape)
    torch.nn.init.normal_(torch_tensor, mean=mean, std=std)
...
@pytest.mark.parametrize("shape, gain, mode", [
    ((10000, 10), np.sqrt(5), 'fan_in'),
    ((10000, 10), 1.0, 'fan_out'),
])
...
def test_eye_initializer(shape):
    my_tensor = Tensor.eye(shape[0]).data
    torch_tensor = torch.eye(shape[0]).numpy()
